<template>
    <main style="display: flex; flex-direction: column; min-width: 1500px; overflow: scroll;">
        ...
        <div style="display: flex; flex-direction: row-reverse; overflow: scroll; justify-content: center;">
            <div id="mountNode" style="border-radius: 10px; border: 2px #8a8a8a solid; width: 1200px; height: 800px; margin-right: 20px;">
            </div>
            <div style="display: flex; flex-direction: column; margin-right: 20px; width: 350px;">
                <div style="display: flex; flex-direction: column;">
                    ...
                    <div v-if="algorithm === 'Dijkstra最短路径'" style="margin-top: 10px;">
                        <v-select 
                            variant="outlined" 
                            v-model="dijkstraSource" 
                            label="选择起始节点" 
                            density="compact" 
                            style="max-height: 60px; width: 300px;"
                            :items="nodes">
                        </v-select>
                    </div>
...
export default{
    ...
    computed: {
        distanceItems() {
            // 将 distances 对象转换为数组，便于表格展示
            return Object.keys(this.dijkstraDistances).map(node => ({
                node: node,
    ...
    methods:{
        step(){
            ...
            if(this.algorithm === 'Kruskal最小生成树'){
                ...
                console.log(this.dijkstraDistances)
